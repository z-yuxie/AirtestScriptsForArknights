...
# 分队类型，可选类型直接搜索（按ctrl+f打开搜索框）脚本中：# 可选分队配置 这个注释项下面的配置
teamType = '心胜于物'
...
class BattleStrategy(Strategy):
    ...
    # 参与攻略的干员, 基础性坐标信息, 关卡特定坐标信息
    def challenge(self, agent, basePositions, targetPositions):
        if not exists(Template(r"tpl1664611277007.png", record_pos=(0.399, 0.096), resolution=(2376, 1152))):
            return False
...
class EventStrategy(Strategy):
    ...
    # 参与攻略的干员, 基础性坐标信息, 关卡特定坐标信息
    def challenge(self, agent, basePositions, targetPositions):
        if not exists(Template(r"tpl1664611435029.png", record_pos=(0.407, 0.088), resolution=(2376, 1152))):
            return False
